...
const TaskCard: React.FC<TaskCardProps> = React.memo(({ task, onEdit, onDelete, onToggleCompleted }) => {
  ...
  const [confirmDelete, setConfirmDelete] = useState(false);  // Estado para confirmación
  ...
  const closeModal = () => {
    ...
    setConfirmDelete(false);  // Reseteamos confirmDelete al cerrar el modal
  };
  ...
  const handleDelete = () => {
    if (confirmDelete) {
      ...
      setConfirmDelete(true);  // Mostrar confirmación antes de eliminar
    }
  };
      ...
      {task.image && (
        <div className="relative mt-4">
          <motion.img
            ...
            className="w-48 h-50 object-cover rounded-lg shadow-md"
      ...
      >
        <div className="bg-violet-500 bg-opacity-80 p-6 rounded-lg shadow-lg w-80 relative">
          ...
          <div className="flex space-x-3 mt-6">
            <button
              onClick={handleDelete}  // Usamos la nueva lógica de confirmación
